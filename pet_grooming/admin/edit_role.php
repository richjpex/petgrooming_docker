...
<?php
   ...
   $stmt = $conn->prepare("SELECT * FROM tbl_groups WHERE id='" . $_POST['id'] . "'");
   $stmt->execute();
                        ...
                        <div class="row">
                           ...
                           <div class="checkbox col-md-12 mt-4">
                              ...
                              <input type="checkbox" id="checkItem" name="checkItem[]" value="<?php echo $id; ?>" 
                                 <?php
                                    $stmt1 = $conn->prepare("select * from tbl_permission_role where group_id='" . $_POST['id'] . "' AND permission_id='$id'");
                                    $stmt1->execute();
                           ...
                           <?php
                              ...
                              $permissions2 = $stmt2->fetchAll();
                              foreach ($permissions2 as $row2) {
                              
                              ?>
                           <div class="checkbox col-md-3">
                              <label>
                              <input type="checkbox" id="checkItem" name="checkItem[]" value="<?php echo $row2['id']; ?>" 
                                 <?php
                                    $stmt3 = $conn->prepare("select * from tbl_permission_role where group_id='" . $_POST['id'] . "' AND permission_id='".$row2['id']."'");
                                    $stmt3->execute();
                                    $row3 = $stmt3->fetch(PDO::FETCH_ASSOC);
                                    
                                    if ($row2['id'] == $row3['permission_id']) {
                                                                                                                                       echo "checked";
                                                                                                                                   } ?>>
